...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    if strcmp(eig_impl, 'JDQR')
        ...
        [laplX, lambda] = jdqr(L, initial_dims + 1, 'sa', options);          % only need bottom (initial_dims + 1) eigenvectors
    else
        options.disp = 0;
        options.isreal = 1;
        options.issym = 1;
        [laplX, lambda] = eigs(L, initial_dims + 1, 'sa', options);			 % only need bottom (initial_dims + 1) eigenvectors
    end
