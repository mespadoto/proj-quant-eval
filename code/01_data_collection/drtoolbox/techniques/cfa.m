...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    if ~exist('no_analyzers', 'var')
        ...
        no_analyzers = str2num(no_analyzers);
    end

    if ~exist('no_iterations', 'var')
        no_iterations = 200;
    else
       no_iterations = str2num(no_iterations); 
    end

    typeinfo(no_analyzers)
    ...
    while iter < no_iterations
        ...
        for j=1:c
            ...
            tmpy = inv(Vc(:,:,j)) * Lambda(:,:,j)';
            m(:,j) = Kappa(:,j) + tmpy * invPsi(:,:,j) * mean(Xnc, 2);
        end
        ...
        for j=1:c
            ...
            tmpx = SigmaC(:,:,j) + eps * eye(d);
            Lambda(:,:,j) = Sc * inv(tmpx);
        end
